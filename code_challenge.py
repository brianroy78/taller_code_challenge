...
class User:
    ...
    def substract_from_balance(self, amount):
        self.balance -= float(amount)
    ...
    def pay_with_balance(self, target, amount, note):
        ...
        self.substract_from_balance(amount)
        payment = Payment(amount, self, target, note)
        target.add_to_balance(amount)
        return payment
...
class TestUser(unittest.TestCase):
    ...
    def test_pay_happy_path(self):
        app = MiniVenmo()
        user_a = app.create_user('user_a_username', 100., '4111111111111111')
        user_b = app.create_user('user_b_username', 100., '4242424242424242')
        user_a.pay(user_b, 55, 'Sandwich')
        self.assertEquals()
        

class TestMiniVenmo(unittest.TestCase):
    def test_create_user_bad_user_name_user_name_exception_rises(self):
        with self.assertRaises(UsernameException):
            app = MiniVenmo()
            app.create_user('!!!!][][]', .0, '123')

    def test_create_user_good_user_name_user_name_exception_not_rises(self):
        app = MiniVenmo()
        app.create_user('normal_username', .0, '123')
        self.assertEquals(app.users[-1].username, 'normal_username')
